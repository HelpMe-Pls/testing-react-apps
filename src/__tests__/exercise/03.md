...
# What I've learnt
- Avoid implementation details (i.e. details that none of our users should
know or care about) and make the test as isolated & straight to the point as possible by using the [`screen`](https://testing-library.com/docs/dom-testing-library/api-queries#screen) object.
- Abstract away the implementation details of an event (e.g. `click`) by using [`userEvent`](https://github.com/HelpMe-Pls/testing-react-apps/blob/master/src/__tests__/final/TS/03.tsx) from the `@testing-library/user-event`.
