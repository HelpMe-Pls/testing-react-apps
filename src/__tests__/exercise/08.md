...
create a component that uses it and then [test that component](https://github.com/HelpMe-Pls/testing-react-apps/blob/master/src/__tests__/final/TS/08.tsx) instead.
- Have your test isolates the custom hook's functionality with a `null` component.
- Use `renderHook` from `@testing-library/react-hooks` to create test for custom hooks that allows the hook's user to [customize its `initialProps`](https://github.com/HelpMe-Pls/testing-react-apps/blob/master/src/__tests__/final/TS/08.extra-3.tsx) instead of using its default value.
