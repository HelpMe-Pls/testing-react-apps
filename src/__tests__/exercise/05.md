...
## What I've learnt
- Use [`msw`](https://mswjs.io/) as a request interceptor for testing.
- Use `msw` as a offline module to write UI for APIs that aren't finished yet.
- Use [`toMatchInlineSnapshot()`](https://github.com/HelpMe-Pls/testing-react-apps/blob/master/src/__tests__/final/TS/05.tsx) rather than an explicit assertion on an error element to keep your tests up-to-date if the error message were to change in the future.
- Colocating run-time server behavior tests by using `server.use`. For such cases, remember to add `server.resetHandlers()` to preserve test isolation and restore the original handlers for other tests.
